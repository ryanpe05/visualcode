...
def main():
	...
	ctx = client.parse("visualCode.py")
	classes = ctx.filter("//uast:Function")
	class_list = {}
	count = 0
	for ind_class in classes:
		class_list[str(count)] = ind_class.get()
		count += 1
	with open("functions.txt", "w+") as fh:
		json.dump(class_list, fh, indent=4)
	ast = ctx.get_all()
	with open("babelStyle.txt", "w+") as fh:
		json.dump(ast, fh, indent=4)
